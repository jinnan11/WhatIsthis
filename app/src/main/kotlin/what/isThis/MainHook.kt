                ...
                DexKitBridge.create(lpparam.appInfo.sourceDir).use { bridge ->
                    ...
                    bridge.findMethod {
                        matcher {
                            usingStrings("%.2f")
                            paramCount(3)
                            paramTypes(Context::class.java, Int::class.java, Int::class.java)
                        }
                    ...
                    val resultMap = bridge.batchFindMethodUsingStrings {
                        groups(map)
                    }

                    val drmResult = resultMap["DrmResult"]!!
                    assert(drmResult.size == 1)
                    val drmResultDescriptor = drmResult.single()
                    ...
                    val largeIconDescriptor = largeIcon.single()
            ...
            "com.miui.personalassistant" -> {
                try {
                    ...
                    loadClass("com.miui.personalassistant.picker.business.detail.PickerDetailViewModel").methodFinder()
                        .filterByName("shouldCheckMamlBoughtState").first().createHook {
                            returnConstant(false)
                        }
